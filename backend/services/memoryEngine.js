...
exports.updateRelationshipMetrics = async (relationshipId) => {
  try {
    ...
    const depthScore =
      intensityValues.length > 0
        ? Math.min(
            ...
            Math.max(
              1,
              intensityValues.reduce((sum, val) => sum + val, 0) /
    ...
    const topicCounts = {
      ...
      values: memoryNodes.filter(
        (node) => node.tags && node.tags.includes("values")
      ).length,
      other: memoryNodes.filter(
        (node) =>
          !["conflict", "support", "joy"].includes(node.type) &&
          !(node.tags && node.tags.includes("values"))
      ).length,
    };

    // Calculate total for percentage conversion
    const totalTopics = Object.values(topicCounts).reduce(
      (sum, val) => sum + val,
      0
    );
...
exports.compressRedundantMemories = async (relationshipId) => {
  try {
    ...
    // Get all memory nodes for this relationship
    const memoryNodes = await MemoryNode.find({
    ...
    for (const type in groupedByType) {
      ...
      if (nodes.length > 3) {
        ...
        for (const node of nodes) {
          if (
            node._id.toString() !== mostIntense._id.toString() &&
