 ...
 */
const generateDetailedMemories = async (
  userId,
  relationshipId,
  conversationId,
  conversationText,
  ...
  try {
    console.log(
      `Generating detailed memories for relationship ${relationshipId}`
    );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert relationship analyst that extracts meaningful memory nodes from conversations.",
        },
        {
          role: "user",
    ...
    try {
      ...
      if (
        ...
        Array.isArray(memories.positiveMemories)
      ) {
      ...
      console.log(
        `Successfully generated detailed memories for relationship ${relationshipId}`
      );
      return true;
    } catch (parseError) {
