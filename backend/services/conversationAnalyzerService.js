  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        if (
          ...
          if (responseTime > 0 && responseTime < 86400000) {
            // Ignore responses over 24 hours
      ...
      const avgResponseTime =
        patterns.responseTime.length > 0
          ? patterns.responseTime.reduce((sum, time) => sum + time, 0) /
            patterns.responseTime.length
          : 0;
      const avgMessageLength =
        patterns.messageLength.length > 0
          ? patterns.messageLength.reduce((sum, len) => sum + len, 0) /
            patterns.messageLength.length
          : 0;
      const questionRatio = patterns.questionFrequency / (messageCount || 1);
      patterns.initiationRatio =
        conversationStarts.user /
        (conversationStarts.user + conversationStarts.contact || 1);

      // Calculate communication balance
      const messageRatio = userMessageCount / (contactMessageCount || 1);
      const communicationBalance =
      ...
      if (
        ...
        const categoryTotal = Object.values(categoryTopicCounts).reduce(
          (sum, count) => sum + count,
          0
        );
        if (categoryTotal > 0) {
      ...
      if (totalPercentage !== 100) {
        ...
        const adjustedTotal = topicDistribution.reduce(
          (sum, topic) => sum + topic.percentage,
          0
        );
        if (adjustedTotal !== 100) {
          const diff = 100 - adjustedTotal;
          const largestTopic = topicDistribution.reduce(
            (max, topic) => (max.percentage > topic.percentage ? max : topic),
            { percentage: 0 }
          );
      ...
      if (
        analysisResult.theirValues &&
        Array.isArray(analysisResult.theirValues)
      ) {
        relationship.theirValues = analysisResult.theirValues;
      }
      if (
        analysisResult.theirInterests &&
        Array.isArray(analysisResult.theirInterests)
      ) {
      ...
      if (
        analysisResult.importantDates &&
        Array.isArray(analysisResult.importantDates)
      ) {
...
const validateAndFixAnalysisResult = (analysisResult) => {
  ...
  requiredFields.forEach((field) => {
    if (!analysisResult[field]) {
      ...
      if (field === "topTopics") {
        ...
        field === "keyInsights" ||
        field === "areasForGrowth" ||
        field === "challengesBadges"
      ) {
        analysisResult[field] = [
          `Analysis didn't provide information about ${field}`,
        ];
  ...
  if (!Array.isArray(analysisResult.keyInsights)) {
    if (typeof analysisResult.keyInsights === "string") {
      ...
      analysisResult.keyInsights = [
        "The conversation shows meaningful exchanges",
      ];
    }
  }
  ...
  if (!Array.isArray(analysisResult.areasForGrowth)) {
    if (typeof analysisResult.areasForGrowth === "string") {
      ...
      analysisResult.areasForGrowth = [
        "Consider building on existing communication patterns",
      ];
    }
  }
