...
class AIEngine {
  ...
  _buildSystemPrompt(relationshipData, phase, previousMessages = []) {
    ...
    if (memoryNodes.length > 0) {
      ...
      const relevantMemories = memoryEngine.getRelevantMemories(
        memoryNodes,
        previousMessages
      );
  ...
  async processUserInput(userInput, conversation, relationship) {
    try {
      ...
      const messages = this._buildMessageHistory(
        conversation,
        relationship,
        phase
      );
      ...
      const memories = this._extractMemories(
        userInput,
        response,
  ...
  _extractInsights(userInput, aiResponse) {
    ...
    const patterns = [
      ...
      {
        regex:
          /(?:feel|feeling|felt) (?:hurt|sad|angry|happy|joyful|grateful)/i,
  ...
  _extractMemories(userInput, aiResponse, contactName) {
    ...
    const memoryPatterns = [
      {
        regex: new RegExp(
          `(?:remember when|that time when|we once|${contactName} and I)`,
          "i"
        ),
        type: "event",
      },
      ...
      for (const pattern of memoryPatterns) {
        if (pattern.regex.test(sentence)) {
          ...
          if (/(?:love|happy|joy|grateful|amazing)/i.test(sentence)) {
            ...
            /(?:sad|angry|hurt|disappointed|frustrated)/i.test(sentence)
          ) {
            emotion = "negative";
          }
  ...
  async generateConversationSummary(conversation, relationship) {
    try {
      ...
      const overallSentiment =
        await sentimentAnalysis.analyzeConversationSentiment(
          messages
  ...
  _selectTopItems(items, count) {
    return [...items]
      .sort(
        (a, b) =>
          (b.confidence || b.weight || 0) - (a.confidence || a.weight || 0)
      )
      .slice(0, count);
  }
