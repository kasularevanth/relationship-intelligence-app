...
const extractDate = (dateString, format) => {
  console.log(
    `Attempting to parse date: "${dateString}" with format: ${format}`
  );
  ...
  try {
    ...
    if (format === "whatsapp-standard") {
      ...
      if (isNaN(dateObj.getTime())) {
        console.warn(
          `Invalid date created from parts: Y=${year}, M=${month}, D=${day}, h=${hours}, m=${minutes}, s=${seconds}`
        );
        return new Date(); // Fallback
      }
      ...
      if (!datePart || !timePart) {
        console.warn(
          `Invalid international date format, missing parts: "${dateString}"`
        );
        return new Date(); // Fallback
      }
      ...
      if (isNaN(dateObj.getTime())) {
        console.warn(
          `Invalid international date created from parts: Y=${year}, M=${month}, D=${day}, h=${hours}, m=${minutes}, s=${seconds}`
        );
        return new Date(); // Fallback
      }
      ...
      try {
        ...
        if (!datePart || !timePart) {
          console.warn(
            `Invalid iMessage date format, missing parts: "${dateString}"`
          );
          return new Date(); // Fallback
        }
        ...
        if (isNaN(dateObj.getTime())) {
          console.warn(
            `Invalid iMessage date created from parts: Y=${year}, M=${month}, D=${day}, h=${hours}, m=${minutes}, s=${seconds || 0}`
          );
          return new Date(); // Fallback
        }
...
const parseWhatsApp = (fileContent, contactPhone = "") => {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (isNaN(timestamp.getTime())) {
        console.warn(
          `Skipping message with invalid timestamp: "${date}, ${time}"`
        );
  ...
  console.log(
    `Matched ${matchCount} messages, couldn't match ${nonMatchCount} lines`
  );
...
const parseWhatsAppInternational = (fileContent, contactPhone = "") => {
  ...
  const messageRegex =
    /^(\d{1,2}\/\d{1,2}\/\d{2}),\s+(\d{1,2}:\d{1,2}\s*(?:am|pm)?)\s*-\s*([^:]+):\s+(.+)$/i;
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      // Determine if the message is from the contact
      const isFromContact =
        sender.trim() !== username && !sender.includes("You");

      // Parse timestamp
      const timestamp = extractDate(
        `${date}, ${time}`,
        "whatsapp-international"
      );
...
const parseWhatsAppIOS = (fileContent, contactPhone = "") => {
  ...
  const messageRegex =
    /^\[(\d{2}\/\d{2}\/\d{2}),\s+(\d{1,2}:\d{2}:\d{2}\s*(?:AM|PM|am|pm))\]\s+([^:]+):\s*(.*)/;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match) {
      ...
      if (
        cleanedText.includes("Messages and calls are end-to-end encrypted") ||
  ...
  console.log(
    `Parsed ${messages.length} messages using iOS format (matched ${matchCount}, continuation lines ${nonMatchCount})`
  );
  console.log("Sample non-matching lines:", sampleNonMatches);
...
const parseChat = (content, contactPhone = "") => {
  ...
  if (format === "whatsapp") {
    ...
    const internationalMessages = parseWhatsAppInternational(
      content,
      contactPhone
    );
...
const parseWhatsAppSample = (fileContent, contactPhone = "") => {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      // Sender logic
      const isFromContact =
        sender.trim() !== "You" && !sender.toLowerCase().includes("you");
  ...
  console.log(
    `Parsed ${messages.length} messages using sample-specific format`
  );
...
const detectChatFormat = (content) => {
  ...
  if (
    sample.match(/\[\d{1,2}\/\d{1,2}\/\d{2},\s+\d{1,2}:\d{2}:\d{2}\s+[AP]M\]/i)
  ) {
    return "whatsapp";
  }
  ...
  if (
    sample.match(/\d{1,2}\/\d{1,2}\/\d{2,4},\s+\d{1,2}:\d{2}\s+-/) ||
