...
exports.getNextQuestion = async (phase, contactName, previousMessages) => {
  ...
  const followUpAsked = aiMessages.some(
    (msg) => msg.content === genericFollowUp
  );
...
function getPhaseQuestions(phase, contactName) {
  ...
  switch (phase) {
    ...
    default:
      return [`Tell me more about your relationship with ${contactName}.`];
  }
}
...
exports.determineNextPhase = (phase, messageCount, conversationData) => {
  console.log("determineNextPhase called with:", {
    phase,
    messageCount,
    conversationData,
  });
  ...
  if (!conversationData || !conversationData.contactName) {
    console.error(
      "Missing contactName in determineNextPhase:",
      conversationData
    );
    // Provide a fallback
    conversationData = conversationData || {};
    conversationData.contactName =
      conversationData.contactName || "your contact";
  }
  ...
  if (messageCount >= phaseThresholds[phase]) {
    ...
    console.log(
      `Moving from ${phase} to ${nextPhase} based on message count ${messageCount}`
    );
    return nextPhase;
  }

  console.log(
    `Phase decision: ${phase} → ${phases[currentPhaseIndex + 1] || phase}`
  );
