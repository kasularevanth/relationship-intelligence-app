...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Relationship synchronized successfully" });
    } catch (error) {
      console.error("Relationship webhook error:", error);
      res
        .status(500)
        .json({ message: "Failed to sync relationship from Firebase" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Conversation synchronized successfully" });
    } catch (error) {
      console.error("Conversation webhook error:", error);
      res
        .status(500)
        .json({ message: "Failed to sync conversation from Firebase" });
    }
  }
);
