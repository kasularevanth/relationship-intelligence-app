...
exports.checkForBias = (message) => {
  ...
  const hasAbsolutes = absoluteTerms.some((term) =>
  ...
  const hasEmotionalHeat = heatTerms.some((term) =>
    lowerMessage.includes(term)
  );

  // Check for one-sided pronoun usage
  const wordCount = message.split(/\s+/).length;
  const theyCount = (lowerMessage.match(/\bthey\b|\bthem\b|\btheir\b/g) || [])
    .length;
  const iCount = (lowerMessage.match(/\bi\b|\bme\b|\bmy\b|\bmine\b/g) || [])
    .length;
