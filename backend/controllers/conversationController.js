...
exports.startConversation = async (req, res) => {
  try {
    ...
    const initialMessage = await conversationFlow.getInitialMessage(
...
async function generateSummary(conversation, messages) {
  try {
    ...
    const keyInsightMatch = summaryText.match(
      /1\.\s*KEY INSIGHTS:?([\s\S]*?)(?=2\.\s*EMOTIONAL DYNAMICS|$)/i
    );
    const emotionalDynamicsMatch = summaryText.match(
      /2\.\s*EMOTIONAL DYNAMICS:?([\s\S]*?)(?=3\.\s*AREAS FOR GROWTH|$)/i
    );
    const areasForGrowthMatch = summaryText.match(
      /3\.\s*AREAS FOR GROWTH:?([\s\S]*?)$/i
    );

    const summary = {
      keyInsights: keyInsightMatch
        ? keyInsightMatch[1].trim()
        : "No key insights provided.",
      emotionalDynamics: emotionalDynamicsMatch
        ? emotionalDynamicsMatch[1].trim()
        : "No emotional dynamics analysis provided.",
      areasForGrowth: areasForGrowthMatch
        ? areasForGrowthMatch[1].trim()
        : "No growth suggestions provided.",
    };
    ...
    return {
      ...
      emotionalDynamics:
        "The emotional dynamics couldn't be fully analyzed due to a technical issue.",
      areasForGrowth:
        "Please try again later for personalized growth suggestions.",
    };
  }
}
