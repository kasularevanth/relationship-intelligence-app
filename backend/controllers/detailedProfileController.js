...
const getDetailedProfile = async (req, res) => {
  try {
    ...
    if (
      !responseData.theirValues ||
      !Array.isArray(responseData.theirValues) ||
      responseData.theirValues.length === 0
    ) {
      responseData.theirValues = ["No values identified yet"];
    }

    if (
      !responseData.theirInterests ||
      !Array.isArray(responseData.theirInterests) ||
      responseData.theirInterests.length === 0
    ) {
    ...
    if (
      ...
      !Array.isArray(responseData.importantDates) ||
      responseData.importantDates.length === 0
    ) {
      responseData.importantDates = [];
    }
    ...
    if (
      ...
      !Array.isArray(responseData.topicDistribution) ||
      responseData.topicDistribution.length === 0
    ) {
      responseData.topicDistribution = [
