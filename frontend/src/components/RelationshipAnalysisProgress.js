 ...
 */
const RelationshipAnalysisProgress = ({
  relationshipId,
  conversationId,
  onComplete,
}) => {
    ...
    const checkAnalysisStatus = async () => {
      try {
        if (attempts >= maxAttempts) {
          setStatus("timeout");
          setError(
            "Analysis is taking longer than expected. You can try refreshing manually."
          );
          return;
        }
        ...
        const response = await relationshipAnalysisService.getDetailedProfile(
          relationshipId
        );
        ...
        setError(
          "Error checking analysis status. Please try refreshing manually."
        );
        setStatus("error");
      }
    };
