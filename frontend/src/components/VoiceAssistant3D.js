...
const VoiceAssistant3D = ({
    ...
    const animate = () => {
      ...
      if (
        ...
        for (let i = 0; i < count; i++) {
          ...
          const speechFactor =
            status === "speaking" ? 0.5 + amplitude * 1.5 : 1.0;

          // Apply enhanced sine wave movement to particles
          positions.array[i3] =
            x + Math.sin(Date.now() * 0.001 + i * 0.1) * 0.01 * speechFactor;
          positions.array[i3 + 1] =
            y + Math.cos(Date.now() * 0.001 + i * 0.1) * 0.01 * speechFactor;
          positions.array[i3 + 2] =
            z + Math.sin(Date.now() * 0.001 + i * 0.05) * 0.01 * speechFactor;
        }
          ...
          onstart: () => {
            ...
            const simulateSpeechPattern = () => {
              ...
              for (let i = 0; i < bufferLength; i++) {
                ...
                const pauseFactor = Math.max(
                  0,
                  Math.sin(time * 1.5) * 0.7 + 0.3
                );
                const wordFactor = Math.max(
                  0,
                  Math.sin(time * 4.0 + i * 0.2) * 0.5 + 0.5
                );

                // Combine factors for a natural speech pattern
                dataArray[i] = Math.floor(
                  baseFactor * pauseFactor * wordFactor * 255
                );
              }
  ...
  useEffect(() => {
    if (!orbitRef.current || !particlesRef.current || !waveformRef.current)
      return;
