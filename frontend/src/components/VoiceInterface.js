...
const VoiceInterface = ({ conversationId, onMessageReceived }) => {
  ...
  const processRecording = async (blob) => {
    try {
      ...
      const stopResponse = await voiceService.stopRecording(
        conversationId,
        formData
      );
      ...
      const checkTranscript = async () => {
        ...
        try {
          const transcriptResponse = await voiceService.getTranscript(
            recordingId
          );
  ...
  return (
    <Box sx={{ textAlign: "center", py: 2 }}>
      <WaveContainer isActive={isRecording}>{renderWaveBars()}</WaveContainer>
