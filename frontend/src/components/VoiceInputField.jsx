...
const VoiceInputField = (props) => {
  const { value, onChange, name, label, ...otherProps } = props;
  ...
  const handleContactPicker = async () => {
    ...
    if (!("contacts" in navigator && "ContactsManager" in window)) {
      ...
      alert(
        "Contact selection is not supported in this browser. Please enter the name manually."
      );
      return;
    }
        ...
        endAdornment: (
          <InputAdornment position="end">
            {isMobile && isNameField ? (
              ...
              <IconButton edge="end" onClick={handleVoiceInput}>
                <MicIcon />
              </IconButton>
            )}
          </InputAdornment>
