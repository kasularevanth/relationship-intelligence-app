...
const PhaseIndicator = styled(Box)(({ theme, active }) => ({
  ...
  borderRadius: 16,
  backgroundColor: active ? "rgba(74, 107, 245, 0.8)" : "rgba(50, 50, 50, 0.6)",
  ...
  cursor: "pointer",
  boxShadow: active ? "0 0 10px rgba(74, 107, 245, 0.7)" : "none",
    ...
    if (messageData.aiResponse && messageData.aiResponse.phase) {
      ...
      if (currentIndex >= 0 && currentIndex < phases.length - 1) {
        ...
        if (currentThreshold && totalMessages >= currentThreshold) {
          ...
          console.log(
            `Moving to ${nextPhase} based on message count ${totalMessages}`
          );
  ...
  return (
    <Container
      ...
      sx={{
        ...
        color: "#fff",
        backgroundImage:
          "radial-gradient(circle at 50% 50%, rgba(30, 30, 50, 0.4) 0%, rgba(0, 0, 0, 0.95) 100%)",
      }}
    >
      <ConversationHeader>
        <HeaderContent>
      ...
      >
        ...
        <DialogContent>
          {summaryLoading ? (
            ...
            <DialogContentText>
              No summary data available. Continue the conversation to generate
              more insights.
            </DialogContentText>
          )}
        </DialogContent>
